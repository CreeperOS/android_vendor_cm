...
# Version 1.0-beta, Adapted for CreeperOS.
...
if [ $ARCH = "64" ]; then
  # Get build version
  MAJOR=$(cat $DIR/vendor/$VENDOR/vendor.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
  MINOR=$(cat $DIR/vendor/$VENDOR/vendor.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
  MAINTENANCE=$(cat $DIR/vendor/$VENDOR/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
  TAG=$(cat $DIR/vendor/$VENDOR/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
  ...
  echo -e "${cya}Building ${bldcya}CreeperOS $VERSION for $DEVICE ${txtrst}";
  ...
  if [ -n "${INTERACTIVE}" ]; then
          ...
          if [ "${VENDOR}" == "creeper" ]; then
                  echo -e "[${bldgrn}lunch creeper_$DEVICE-userdebug${bldblu}]${txtrst}"
          else
          ...
          lunch "creeper_$DEVICE-userdebug";
